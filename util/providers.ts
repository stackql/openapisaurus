...
export function updateResourceName({ providerName, service, inResourceName, operation, debug, logger }: { providerName: string, service: string, inResourceName: string, operation: any, debug: boolean, logger: any }): string {
    debug && logger.debug(`checking for resource name updates for [${inResourceName}] in [${providerName}.${service}]...`);
    let outResourceName = inResourceName;
    if (providerName in providers) {
        debug && logger.debug(`provider data found for ${providerName}`);
        if (service in providers[providerName].resourcesMap) {
            debug && logger.debug(`service data found for ${providerName}.${service}`);
            ...
            if (!nameFromOpIdOrNull) {
                ...
                if (nameFromExistingOrNull) {
                    ...
                    debug && logger.debug(`resource name changed from ${inResourceName} to ${outResourceName} via name mapping`);
                }
            } else {
                outResourceName = nameFromOpIdOrNull;
                debug && logger.debug(`resource name changed from ${inResourceName} to ${outResourceName} via operationId`);
            }
        }
    }
