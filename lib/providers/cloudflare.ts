...
export const stackqlMethodNameMap = {
    ...
    methodNameTransforms: {
        allServices: (service: string, resource: string, operationId: string, tag: string) => {
            // Convert the tag to match the format in operationId
            let convertedTag = tag.toLowerCase().replace(/\s/g, '-').replace(/\(/g, '(-').replace(/\)/g, '-)');
            
            // Remove the converted tag from the operationId
            let methodName = operationId.replace(convertedTag, '').replace(/^-/, '');
